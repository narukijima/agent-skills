...
Remote media remains mutable after prepare. Facebook video acceptance is `submitted`; use the status surface and native status object before calling it fully published. If a timeout occurs without a stable object ID, reconciliation cannot prove absence and remains unresolved.
