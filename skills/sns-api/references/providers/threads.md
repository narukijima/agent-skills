...
Remote media is fetched by Threads and may mutate after prepare. The Skill records URL/host/expected MIME but does not claim byte-level assurance. If a submitted state outlives its manifest, use a fresh state-bound `authorize-resume` approval rather than extending the old signature.
