...
Do not retry timeout, disconnect, authenticated redirect, 5xx, malformed success, or missing Provider ID. Run `reconcile`. A definite 4xx becomes `failed`; one exact-intent retry may reuse the same Domain Authorization reference and remains bounded by the attempt and call budgets. After the two-attempt cap for one Domain Authorization reference, a definite `failed`/`confirmed_absent` may be retried only by preparing a manifest under a new explicit Domain Authorization; the ledger records an audited `retry-authorized` event with the previous reference and attempt count. `unknown` never gains this path.

A 429 does not consume the publish attempt but still consumes the conservative daily call reservation. The Skill persists the provider-communicated reset (`x-rate-limit-reset`/`retry-after`, defaulting to the official 15-minute window) and refuses further sends/reads for that platform, locally and before any billable call, until the window passes. This matches the official guidance to wait for the reset instead of re-sending, and it stops retry loops from spending pay-per-use credits on requests that cannot succeed.
