...
VERSION = "0.3.0"
SCHEMA_VERSION = "origen-evidence/3"
POLICY_VERSION = "origen-trust-policy/1"
...
POLICY_FIELDS = {
    "schema_version", "policy_id", "policy_version", "mode", "root_required",
    "human_origin_claim", "allowed_media_types", "approved_signers",
    "approved_verifiers", "approved_builders", "approved_inspectors",
    "approved_timestamp_providers", "approved_key_ids", "approved_algorithms",
    "creator_key_map", "resource_limits", "environment_policy",
...
TOOL_POLICY_FIELDS = {
    ...
    "verifier_identity", "builder_identity", "inspector_identity", "agent_invocable",
}
...
EVIDENCE_FIELDS = {
    ...
    "publication", "publish_ready", "proof",
}
...
def validate_depth(value: object, maximum: int, depth: int = 0) -> None:
    if depth > maximum:
        raise V3Error("JSON_DEPTH_EXCEEDED", "JSON nesting exceeds Policy limit", maximum=maximum)
...
def load_strict_json_bytes(data: bytes, core: Any, *, label: str, max_depth: int) -> dict[str, object]:
    try:
        ...
        raise V3Error("INVALID_JSON", f"{label} is not strict UTF-8 JSON") from error
    except core.OrigenError as error:
        raise V3Error(error.code, error.message, **error.details) from error
    if not isinstance(value, dict):
        raise V3Error("INVALID_JSON", f"{label} root must be an object")
...
class SnapshotStore:
    ...
    def _copy_fd(self, fd: int, *, label: str, maximum: int, name_hint: str) -> Snapshot:
        ...
        if not stat.S_ISREG(before.st_mode):
            raise V3Error("NOT_REGULAR_FILE", f"{label} must be a regular file")
        if before.st_size > maximum:
            raise V3Error("FILE_TOO_LARGE", f"{label} exceeds Policy byte limit", size=before.st_size, maximum=maximum)
        ...
        try:
            with os.fdopen(temporary_fd, "wb", closefd=True) as output:
                while True:
                    ...
                    copied += len(chunk)
                    if copied > maximum:
                        raise V3Error("FILE_TOO_LARGE", f"{label} exceeds Policy byte limit", maximum=maximum)
                    digest.update(chunk)
                    output.write(chunk)
                output.flush()
            ...
            if not stable or copied != before.st_size:
                raise V3Error("INPUT_MUTATED", f"{label} changed while the secure snapshot was captured")
    ...
    def capture(self, path: Path, *, label: str, maximum: int) -> Snapshot:
        try:
            ...
            raise V3Error("FILE_NOT_FOUND", f"{label} does not exist", path=str(path)) from error
        if stat.S_ISLNK(info.st_mode):
            raise V3Error("SYMLINK_REJECTED", f"{label} must not be a symlink", path=str(path))
        flags = os.O_RDONLY | getattr(os, "O_CLOEXEC", 0) | getattr(os, "O_NOFOLLOW", 0)
        try:
            fd = os.open(path, flags)
        except OSError as error:
            raise V3Error("SECURE_OPEN_FAILED", f"{label} could not be opened without following links", path=str(path)) from error
        try:
            opened = os.fstat(fd)
            if (opened.st_dev, opened.st_ino) != (info.st_dev, info.st_ino):
                raise V3Error("PATH_SWAPPED", f"{label} path changed between lstat and no-follow open", path=str(path))
    ...
    def capture_child(self, directory: Path, name: str, *, label: str, maximum: int) -> Snapshot:
        try:
            ...
            raise V3Error("BUNDLE_NOT_FOUND", "publish bundle does not exist", path=str(directory)) from error
        if stat.S_ISLNK(info.st_mode) or not stat.S_ISDIR(info.st_mode):
            raise V3Error("INVALID_BUNDLE", "publish bundle must be a non-symlink directory")
        dir_flags = os.O_RDONLY | getattr(os, "O_DIRECTORY", 0) | getattr(os, "O_NOFOLLOW", 0)
        dir_fd = os.open(directory, dir_flags)
        try:
            entries = set(os.listdir(dir_fd))
            if entries != {"asset", "evidence.json", "receipt.json"}:
                raise V3Error("INVALID_BUNDLE", "publish bundle must contain exactly asset, evidence.json, and receipt.json", entries=sorted(entries))
            flags = os.O_RDONLY | getattr(os, "O_CLOEXEC", 0) | getattr(os, "O_NOFOLLOW", 0)
            try:
                fd = os.open(name, flags, dir_fd=dir_fd)
            except OSError as error:
                raise V3Error("INVALID_BUNDLE", f"bundle {name} cannot be opened safely") from error
            try:
...
def normalize_timestamp(value: object, *, label: str) -> str:
    if not isinstance(value, str) or not value:
        raise V3Error("INVALID_TIMESTAMP", f"{label} must be a non-empty RFC3339 timestamp")
    candidate = value[:-1] + "+00:00" if value.endswith("Z") else value
    try:
        parsed = datetime.fromisoformat(candidate)
    except ValueError as error:
        raise V3Error("INVALID_TIMESTAMP", f"{label} must be RFC3339-compatible") from error
    if parsed.tzinfo is None:
        raise V3Error("INVALID_TIMESTAMP", f"{label} must include a timezone")
...
def require_string(mapping: dict[str, object], key: str, context: str) -> str:
    ...
    if not isinstance(value, str) or not value:
        raise V3Error("INVALID_POLICY", f"{context}.{key} must be a non-empty string")
    return value


def require_sha256(value: object, context: str) -> str:
    if not isinstance(value, str) or not SHA256_RE.fullmatch(value):
        raise V3Error("INVALID_POLICY", f"{context} must be a lowercase SHA-256 digest")
    return value
...
def policy_limits(policy: dict[str, object]) -> dict[str, int | float]:
    ...
    if not isinstance(supplied, dict):
        raise V3Error("INVALID_POLICY", "resource_limits must be an object")
    unknown = set(supplied) - set(DEFAULT_LIMITS)
    if unknown:
        raise V3Error("UNKNOWN_POLICY_FIELD", "unknown resource limit", fields=sorted(unknown))
    limits = dict(DEFAULT_LIMITS)
    for key, value in supplied.items():
        if not isinstance(value, (int, float)) or isinstance(value, bool) or value <= 0:
            raise V3Error("INVALID_POLICY", f"resource_limits.{key} must be positive")
        limits[key] = value
    return limits
...
def validate_policy(policy: dict[str, object]) -> None:
    ...
    if unknown:
        raise V3Error("UNKNOWN_POLICY_FIELD", "Trust Policy contains unknown critical fields", fields=sorted(unknown))
    if policy.get("schema_version") != POLICY_VERSION:
        raise V3Error("UNSUPPORTED_POLICY_SCHEMA", "unsupported Trust Policy schema")
    require_string(policy, "policy_id", "policy")
    require_string(policy, "policy_version", "policy")
    mode = policy.get("mode")
    if mode not in POLICY_MODES:
        raise V3Error("INVALID_POLICY", "mode must be development or production")
    root_required = policy.get("root_required", True)
    if not isinstance(root_required, bool):
        raise V3Error("INVALID_POLICY", "root_required must be boolean")
    human_claim = policy.get("human_origin_claim", root_required)
    if not isinstance(human_claim, bool):
        raise V3Error("INVALID_POLICY", "human_origin_claim must be boolean")
    if mode == "production" and not root_required and human_claim:
        raise V3Error("INVALID_POLICY", "rootless Production profile must set human_origin_claim=false")
    ...
    if not isinstance(media, list) or not media or any(not isinstance(item, str) or not item for item in media):
        raise V3Error("INVALID_POLICY", "allowed_media_types must be a non-empty string list")
    for field in ("approved_signers", "approved_verifiers", "approved_builders", "approved_inspectors", "approved_timestamp_providers"):
        value = policy.get(field, {})
        if not isinstance(value, dict):
            raise V3Error("INVALID_POLICY", f"{field} must be an object keyed by approved ID")
    for field in ("approved_key_ids", "approved_algorithms"):
        value = policy.get(field)
        if not isinstance(value, list) or not value or any(not isinstance(item, str) or not item for item in value):
            raise V3Error("INVALID_POLICY", f"{field} must be a non-empty string list")
    if not isinstance(policy.get("creator_key_map"), dict):
        raise V3Error("INVALID_POLICY", "creator_key_map must be an object")
    ...
    if not isinstance(environment, dict) or environment.get("network") not in {"deny", "explicit"}:
        raise V3Error("INVALID_POLICY", "environment_policy.network must be deny or explicit")
    ...
    if not isinstance(approved_path, list) or any(not isinstance(item, str) or not Path(item).is_absolute() for item in approved_path):
        raise V3Error("INVALID_POLICY", "environment_policy.approved_path must contain absolute directories")
    handoff = policy.get("publisher_handoff_policy")
    if not isinstance(handoff, dict):
        raise V3Error("INVALID_POLICY", "publisher_handoff_policy must be an object")
    ...
    if not isinstance(representations, list) or not representations or any(not isinstance(item, str) for item in representations):
        raise V3Error("INVALID_POLICY", "publisher_handoff_policy.publication_representations is required")
    if not isinstance(handoff.get("allowed_transport_metadata", []), list):
        raise V3Error("INVALID_POLICY", "allowed_transport_metadata must be a list")
    slice_policy = policy.get("slice_boundary_policy", {})
    if not isinstance(slice_policy, dict):
        raise V3Error("INVALID_POLICY", "slice_boundary_policy must be an object")
    ...
    if not isinstance(c2pa_policy, dict) or c2pa_policy.get("action", "detach") not in {"preserve", "reissue", "detach"}:
        raise V3Error("INVALID_POLICY", "c2pa_policy.action must be preserve, reissue, or detach")
    for field in ("publication_profiles", "approved_json_schemas"):
        if not isinstance(policy.get(field, {}), dict):
            raise V3Error("INVALID_POLICY", f"{field} must be an object")


def load_policy(snapshot: Snapshot, core: Any) -> tuple[dict[str, object], str, dict[str, int | float]]:
    policy = load_strict_json_bytes(snapshot.read_bytes(), core, label="Trust Policy", max_depth=int(DEFAULT_LIMITS["json_depth"]))
    validate_policy(policy)
    return policy, digest_bytes(canonical_bytes(policy)), policy_limits(policy)


def hash_regular_nofollow(path: Path, *, label: str) -> tuple[str, os.stat_result]:
    if not path.is_absolute():
        raise V3Error("POLICY_PATH_NOT_ABSOLUTE", f"{label} path must be absolute", path=str(path))
    try:
        info = os.lstat(path)
    except FileNotFoundError as error:
        raise V3Error("TOOL_NOT_FOUND", f"{label} does not exist", path=str(path)) from error
    if stat.S_ISLNK(info.st_mode) or not stat.S_ISREG(info.st_mode):
        raise V3Error("UNSAFE_TOOL_PATH", f"{label} must be a regular non-symlink file", path=str(path))
    ...
    try:
        opened = os.fstat(fd)
        if (opened.st_dev, opened.st_ino) != (info.st_dev, info.st_ino):
            raise V3Error("TOOL_PATH_RACE", f"{label} changed before hashing")
        while chunk := os.read(fd, 1024 * 1024):
            digest.update(chunk)
        after = os.fstat(fd)
        if (opened.st_size, opened.st_mtime_ns, opened.st_ctime_ns) != (after.st_size, after.st_mtime_ns, after.st_ctime_ns):
            raise V3Error("TOOL_PATH_RACE", f"{label} changed while hashing")
...
def resolve_tool(policy: dict[str, object], category: str, tool_id: str | None, *, role: str | None = None) -> dict[str, object]:
    if not tool_id:
        raise V3Error("APPROVED_ID_REQUIRED", f"an approved {category} ID is required")
    ...
    if not isinstance(registry, dict) or tool_id not in registry:
        code = {
            ...
            "approved_verifiers": "VERIFIER_NOT_APPROVED",
        ...
        raise V3Error(code, f"{tool_id!r} is not approved by this Trust Policy")
    raw = registry[tool_id]
    if not isinstance(raw, dict):
        raise V3Error("INVALID_POLICY", f"{category}.{tool_id} must be an object")
    entry = dict(raw)
    unknown = set(entry) - TOOL_POLICY_FIELDS
    if unknown:
        raise V3Error("UNKNOWN_POLICY_FIELD", f"{category}.{tool_id} contains unknown critical fields", fields=sorted(unknown))
    ...
    if actual_executable != expected_executable:
        raise V3Error("EXECUTABLE_HASH_MISMATCH", "approved executable hash does not match Policy", tool_id=tool_id)
    if role is not None and entry.get("role") != role:
        raise V3Error("ATTESTOR_ROLE_MISMATCH", f"{tool_id!r} is not approved for {role}")
    arguments = entry.get("arguments", [])
    if not isinstance(arguments, list) or any(not isinstance(item, str) for item in arguments):
        raise V3Error("INVALID_POLICY", f"{category}.{tool_id}.arguments must be a string list")
    ...
    if not isinstance(script_hashes, dict) or not isinstance(resource_hashes, dict):
        raise V3Error("INVALID_POLICY", "expected script/resource hashes must be path-to-digest objects")
    verified_files = []
    for kind, values in (("script", script_hashes), ("resource", resource_hashes)):
        for raw_path, expected in values.items():
            if not isinstance(raw_path, str):
                raise V3Error("INVALID_POLICY", f"expected_{kind}_sha256 keys must be paths")
            ...
            if actual != expected_digest:
                raise V3Error(f"{kind.upper()}_HASH_MISMATCH", f"approved {kind} hash does not match Policy", path=raw_path)
    ...
    for argument in arguments:
        ...
        if candidate.is_absolute() and candidate.exists():
            if str(candidate) not in script_hashes and str(candidate) not in resource_hashes:
                raise V3Error("UNPINNED_TOOL_ARGUMENT", "existing absolute command file is not hash-pinned", path=str(candidate))
    if category == "approved_signers":
        key_id = require_string(entry, "key_id", f"{category}.{tool_id}")
        algorithm = require_string(entry, "algorithm", f"{category}.{tool_id}")
        require_string(entry, "signer_identity", f"{category}.{tool_id}")
        if key_id not in policy["approved_key_ids"]:
            raise V3Error("KEY_ID_NOT_APPROVED", "signer key ID is not approved by Policy", key_id=key_id)
        if algorithm not in policy["approved_algorithms"]:
            raise V3Error("ALGORITHM_NOT_APPROVED", "signer algorithm is not approved by Policy", algorithm=algorithm)
        if role == "root-attestor" and policy.get("mode") == "production" and entry.get("agent_invocable") is not False:
            raise V3Error("ROOT_ATTESTOR_EXPOSED", "Production root-attestor must require an out-of-band authorization boundary")
    entry.update({
        "id": tool_id,
...
def sanitized_environment(policy: dict[str, object], workdir: Path) -> dict[str, str]:
    ...
    if not isinstance(allowed, dict) or any(not isinstance(k, str) or not isinstance(v, str) for k, v in allowed.items()):
        raise V3Error("INVALID_POLICY", "environment_policy.allowed_variables must be a literal string map")
    env.update(allowed)
    return env
...
def run_tool(entry: dict[str, object], request: dict[str, object], *, policy: dict[str, object], limits: dict[str, int | float], operation: str, workdir: Path) -> dict[str, object]:
    ...
    process = subprocess.Popen(
        ...
        cwd=workdir, env=sanitized_environment(policy, workdir), close_fds=True,
    )
    ...
    try:
        while selector.get_map():
            ...
            if remaining <= 0:
                process.kill()
                process.wait()
                raise V3Error("PROVIDER_TIMEOUT", f"{operation} exceeded the Policy timeout")
            ...
            for key, _ in events:
                ...
                if len(target) > caps[key.data]:
                    process.kill()
                    process.wait()
                    raise V3Error("PROVIDER_OUTPUT_LIMIT", f"{operation} exceeded {key.data} byte limit")
        ...
        if process.poll() is None:
            ...
            process.wait()
    if returncode != 0:
        raise V3Error("PROVIDER_FAILED", f"{operation} returned non-zero", returncode=returncode)
    def unique_pairs(pairs: list[tuple[str, object]]) -> dict[str, object]:
        result: dict[str, object] = {}
        for key, value in pairs:
            if key in result:
                raise V3Error("INVALID_PROVIDER_RESPONSE", f"{operation} response contains duplicate keys", key=key)
            result[key] = value
        return result

    def reject_constant(value: str) -> object:
        raise V3Error("INVALID_PROVIDER_RESPONSE", f"{operation} response contains NaN or Infinity", value=value)
    ...
    try:
        ...
        raise V3Error("INVALID_PROVIDER_RESPONSE", f"{operation} did not return one JSON object") from error
    if not isinstance(response, dict):
        raise V3Error("INVALID_PROVIDER_RESPONSE", f"{operation} response must be an object")
    ...
    if actual_after != entry["actual_executable_sha256"] or (
        ...
        raise V3Error("TOOL_MUTATED_DURING_EXECUTION", f"{operation} executable changed during execution")
    return response
...
def sign_statement(statement: dict[str, object], signer: dict[str, object], *, policy: dict[str, object], limits: dict[str, int | float], workdir: Path) -> dict[str, object]:
    ...
    response = run_tool(signer, {
        "operation": "sign", "request_scope": f"sign-{signer['role']}-evidence-v3",
    ...
    for key, expected in (("provider", signer.get("provider", signer["id"])), ("key_id", signer["key_id"]), ("algorithm", signer["algorithm"]), ("signer_identity", signer["signer_identity"])):
        if response.get(key) != expected:
            raise V3Error("SIGNER_IDENTITY_MISMATCH", f"signer returned an unexpected {key}")
    signature = response.get("signature")
    if not isinstance(signature, str) or not signature:
        raise V3Error("INVALID_PROVIDER_RESPONSE", "signer response is missing signature")
    if signer["role"] == "root-attestor" and policy["mode"] == "production":
        authorization = response.get("authorization_receipt_digest")
        signed_authorization = statement.get("event", {}).get("authorization_receipt_digest")
        if not isinstance(authorization, str) or not SHA256_RE.fullmatch(authorization) or authorization != signed_authorization:
            raise V3Error("ROOT_AUTHORIZATION_REQUIRED", "root-attestor did not echo the signed out-of-band authorization receipt digest")
    return {"signature": signature}


def authorize_root(subject: dict[str, object], signer: dict[str, object], *, policy: dict[str, object], limits: dict[str, int | float], workdir: Path) -> str:
    response = run_tool(signer, {
        "operation": "authorize_root", "protocol": "origen-root-authorization/1",
        "subject_sha256": digest_bytes(canonical_bytes(subject)),
    ...
    receipt_digest = response.get("authorization_receipt_digest")
    if not isinstance(receipt_digest, str) or not SHA256_RE.fullmatch(receipt_digest):
        raise V3Error("ROOT_AUTHORIZATION_REQUIRED", "root-attestor authorization provider did not return a receipt digest")
...
def verify_statement(evidence: dict[str, object], verifier: dict[str, object], *, policy: dict[str, object], limits: dict[str, int | float], workdir: Path) -> None:
    ...
    if not isinstance(proof, dict) or not isinstance(proof.get("signature"), str):
        raise V3Error("INVALID_EVIDENCE", "v3 proof must contain a signature")
    statement = {key: value for key, value in evidence.items() if key != "proof"}
    response = run_tool(verifier, {
        "operation": "verify", "payload": base64.b64encode(canonical_bytes(statement)).decode("ascii"),
        "payload_sha256": digest_bytes(canonical_bytes(statement)), "proof": proof,
        "expected_signer": evidence["identities"]["signer"],
    }, policy=policy, limits=limits, operation="verify", workdir=workdir)
    if response.get("verified") is not True:
        raise V3Error("SIGNATURE_INVALID", "v3 evidence signature was not verified")
    ...
    for key in ("key_id", "algorithm", "identity"):
        ...
        if response.get(response_key) != signed.get(key):
            raise V3Error("SIGNATURE_IDENTITY_MISMATCH", f"verifier returned a different {key}")
...
def obtain_trusted_timestamp(subject: dict[str, object], provider: dict[str, object], *, policy: dict[str, object], limits: dict[str, int | float], workdir: Path) -> tuple[dict[str, object], dict[str, object]]:
    ...
    if not isinstance(receipt, str) or not receipt:
        raise V3Error("INVALID_TIMESTAMP_RESPONSE", "timestamp provider response is missing receipt")
    identity = response.get("provider")
    if identity != provider.get("provider", provider["id"]):
        raise V3Error("TIMESTAMP_PROVIDER_MISMATCH", "timestamp provider identity does not match Policy")
    receipt_digest = digest_bytes(receipt.encode("utf-8"))
    statement = {
        "provider_id": provider["id"], "provider_identity": identity,
...
def verify_trusted_timestamp(evidence: dict[str, object], provider: dict[str, object], *, policy: dict[str, object], limits: dict[str, int | float], workdir: Path) -> None:
    ...
    if not isinstance(timestamp, dict) or not isinstance(proof, dict):
        raise V3Error("TRUSTED_TIMESTAMP_REQUIRED", "trusted timestamp statement and receipt are required")
    ...
    if not isinstance(receipt, str) or digest_bytes(receipt.encode("utf-8")) != timestamp.get("receipt_digest"):
        raise V3Error("TIMESTAMP_RECEIPT_TAMPERED", "timestamp receipt digest does not match signed statement")
    response = run_tool(provider, {
    ...
    if response.get("verified") is not True:
        raise V3Error("TIMESTAMP_INVALID", "trusted timestamp receipt was not verified")
...
def detect_c2pa_markers(snapshot: Snapshot, media_type: str, core: Any) -> list[str]:
    ...
    if media_type in {"text/plain", "text/markdown", "text/html", "image/svg+xml"}:
        try:
            ...
            raise V3Error("INVALID_UTF8", "text asset is not strict UTF-8") from error
        start = "-----BEGIN C2PA MANIFEST-----" in text
        end = "-----END C2PA MANIFEST-----" in text
        if start != end:
            raise V3Error("MALFORMED_C2PA_TEXT_WRAPPER", "C2PA text wrapper is incomplete")
        if start:
...
def enforce_png_limits(snapshot: Snapshot, limits: dict[str, int | float], core: Any) -> None:
    ...
    if width > limits["width"] or height > limits["height"]:
        raise V3Error("PNG_DIMENSIONS_EXCEEDED", "PNG dimensions exceed Policy limits", width=width, height=height)
    if width * height > limits["pixel_count"]:
        raise V3Error("PNG_PIXEL_LIMIT", "PNG pixel count exceeds Policy limit", pixels=width * height)
    row_bytes = (width * channels * bit_depth + 7) // 8
    decoded = height * (row_bytes + 1)
    if decoded > limits["decoded_bytes"]:
        raise V3Error("PNG_DECOMPRESSION_LIMIT", "PNG decoded bytes exceed Policy limit", decoded_bytes=decoded)
...
def inspect_snapshot(snapshot: Snapshot, *, policy: dict[str, object], limits: dict[str, int | float], core: Any, publication_profile: str | None = None) -> dict[str, object]:
    ...
    if media_type not in policy["allowed_media_types"]:
        raise V3Error("MEDIA_TYPE_NOT_ALLOWED", "media type is not allowed by Policy", media_type=media_type)
    if media_type == "image/png":
    ...
    if media_type == "application/zip":
        try:
            ...
            raise V3Error("INVALID_ARCHIVE", "ZIP container is malformed") from error
        if len(entries) > limits["archive_entry_count"]:
            raise V3Error("ARCHIVE_ENTRY_LIMIT", "ZIP entry count exceeds Policy limit")
        ...
        if uncompressed > limits["decoded_bytes"] or (uncompressed / compressed if compressed else float("inf")) > limits["compression_ratio"]:
            raise V3Error("ARCHIVE_BOMB", "ZIP expansion exceeds Policy limits")
    ...
    if media_type == "text/markdown":
        ...
        if not publication_profile or not isinstance(profiles, dict) or publication_profile not in profiles:
            raise V3Error("PUBLICATION_PROFILE_REQUIRED", "Markdown requires a Policy publication profile")
        profile = profiles[publication_profile]
        if not isinstance(profile, dict):
            raise V3Error("INVALID_POLICY", "Markdown publication profile must be an object")
...
def validate_typed_operation(value: object, *, source_ids: set[str]) -> dict[str, object]:
    if not isinstance(value, dict) or set(value) - {"op", "parameters"}:
        raise V3Error("INVALID_OPERATION", "typed operation must contain only op and parameters")
    op = value.get("op")
    parameters = value.get("parameters", {})
    if op not in ALLOWED_OPERATIONS or not isinstance(parameters, dict):
        raise V3Error("INVALID_OPERATION", "operation is not allowed by v1 typed schema", operation=op)
    for key, item in parameters.items():
        lowered = str(key).lower()
        if any(fragment in lowered for fragment in UNSAFE_PARAMETER_KEYS):
            raise V3Error("UNSAFE_OPERATION_PARAMETER", "operation parameter is forbidden", parameter=key)
        if isinstance(item, str) and (re.match(r"^[A-Za-z][A-Za-z0-9+.-]*://", item) or "base64," in item.lower()):
            raise V3Error("UNSAFE_OPERATION_PARAMETER", "URL/base64 content is forbidden in operations", parameter=key)
    allowed_parameters = OPERATION_PARAMETERS[op]
    if set(parameters) - allowed_parameters:
        raise V3Error("INVALID_OPERATION", "operation contains parameters outside its typed schema", parameters=sorted(set(parameters) - allowed_parameters))
    ...
    if missing:
        raise V3Error("INVALID_OPERATION", "operation is missing required typed parameters", parameters=sorted(missing))
    ...
    for key, item in parameters.items():
        if isinstance(item, str):
            if re.match(r"^[A-Za-z][A-Za-z0-9+.-]*://", item) or "base64," in item.lower():
                raise V3Error("UNSAFE_OPERATION_PARAMETER", "URL/base64 content is forbidden in operations", parameter=key)
            if len(item) > 1024:
                raise V3Error("UNSAFE_OPERATION_PARAMETER", "freeform content is forbidden in operations", parameter=key)
        elif not isinstance(item, (int, float, bool, list, dict, type(None))):
            raise V3Error("INVALID_OPERATION", "operation parameter has unsupported type", parameter=key)
    if op in CONTENT_BEARING_OPS:
        source_id = parameters.get("source_id")
        if source_id not in source_ids:
            raise V3Error("UNSIGNED_CONTENT_RESOURCE", f"{op} requires a signed source_id")
    for key in ("width", "height", "rate"):
        if key in parameters and (not isinstance(parameters[key], (int, float)) or isinstance(parameters[key], bool) or parameters[key] <= 0):
            raise V3Error("INVALID_OPERATION", f"{op}.{key} must be positive")
    for key in ("x", "y", "start", "end"):
        if key in parameters and (not isinstance(parameters[key], (int, float)) or isinstance(parameters[key], bool) or parameters[key] < 0):
            raise V3Error("INVALID_OPERATION", f"{op}.{key} must be non-negative")
    if "start" in parameters and "end" in parameters and parameters["end"] <= parameters["start"]:
        raise V3Error("INVALID_OPERATION", f"{op} end must be greater than start")
    for key in ("source_ids", "channels"):
        if key in parameters and (not isinstance(parameters[key], list) or not parameters[key]):
            raise V3Error("INVALID_OPERATION", f"{op}.{key} must be a non-empty list")
    if "source_ids" in parameters and any(item not in source_ids for item in parameters["source_ids"]):
        raise V3Error("UNSIGNED_CONTENT_RESOURCE", f"{op} source_ids must reference signed sources")
...
def validate_asset_record(value: object, context: str) -> dict[str, object]:
    if not isinstance(value, dict) or set(value) != {"id", "sha256", "size", "media_type"}:
        raise V3Error("INVALID_EVIDENCE", f"{context} has unknown or missing fields")
    digest = value.get("sha256")
    if not isinstance(digest, str) or not SHA256_RE.fullmatch(digest) or value.get("id") != f"sha256:{digest}":
        raise V3Error("INVALID_EVIDENCE", f"{context} hash identity is invalid")
    if not isinstance(value.get("size"), int) or value["size"] < 0 or not isinstance(value.get("media_type"), str):
        raise V3Error("INVALID_EVIDENCE", f"{context} size/media_type is invalid")
    return value
...
def validate_v3_evidence(evidence: dict[str, object]) -> None:
    unknown = set(evidence) - EVIDENCE_FIELDS
    if unknown:
        raise V3Error("UNKNOWN_EVIDENCE_FIELD", "evidence contains unknown critical fields", fields=sorted(unknown))
    if evidence.get("schema_version") != SCHEMA_VERSION or evidence.get("operation_schema_version") != OPERATION_VERSION:
        raise V3Error("UNSUPPORTED_EVIDENCE_SCHEMA", "Production requires Origen evidence v3")
    evidence_type = evidence.get("evidence_type")
    if evidence_type not in {"human-root", "final-asset"}:
        raise V3Error("INVALID_EVIDENCE", "evidence_type is invalid")
    ...
    if not isinstance(policy, dict) or set(policy) != {"policy_id", "policy_version", "mode", "digest"}:
        raise V3Error("INVALID_EVIDENCE", "signed Policy claim is malformed")
    require_sha256(policy.get("digest"), "evidence.policy.digest")
    identities = evidence.get("identities")
    if not isinstance(identities, dict) or not isinstance(identities.get("signer"), dict) or not isinstance(identities.get("verifier"), dict):
        raise V3Error("INVALID_EVIDENCE", "signed identities are incomplete")
    signer = identities["signer"]
    if set(signer) != {"signer_id", "role", "key_id", "identity", "algorithm"}:
        raise V3Error("INVALID_EVIDENCE", "signed signer identity has unknown or missing fields")
    if signer.get("role") not in SIGNER_ROLES:
        raise V3Error("INVALID_EVIDENCE", "signed attestor role is invalid")
    ...
    if not isinstance(assurance, dict) or set(assurance) != {"structural", "content_signals", "derivation", "root"}:
        raise V3Error("INVALID_EVIDENCE", "assurance dimensions are incomplete")
    for key in ("structural", "content_signals", "derivation", "root"):
        if not isinstance(assurance.get(key), dict):
            raise V3Error("INVALID_EVIDENCE", f"assurance.{key} must be an object")
    if assurance["content_signals"].get("state") not in CONTENT_SIGNAL_STATES:
        raise V3Error("INVALID_EVIDENCE", "content signal state is invalid")
    proof = evidence.get("proof")
    if not isinstance(proof, dict) or not isinstance(proof.get("signature"), str):
        raise V3Error("INVALID_EVIDENCE", "proof signature is missing")
    allowed_proof = {"signature", "authorization_receipt_digest", "timestamp_receipt"}
    if set(proof) - allowed_proof:
        raise V3Error("UNKNOWN_EVIDENCE_FIELD", "proof contains unknown critical fields", fields=sorted(set(proof) - allowed_proof))
    if evidence_type == "human-root":
        if signer.get("role") != "root-attestor" or evidence.get("publish_ready") is not False:
            raise V3Error("INVALID_EVIDENCE", "Human Root requires root-attestor and publish_ready=false")
        if not isinstance(evidence.get("origin"), dict) or not isinstance(evidence.get("timestamp"), dict):
            raise V3Error("INVALID_EVIDENCE", "Human Root origin/timestamp claims are missing")
        event = evidence.get("event")
        if not isinstance(event, dict) or set(event) != {"action", "tool", "version", "authorization_receipt_digest"} or event.get("action") != "human-root-attestation":
            raise V3Error("INVALID_EVIDENCE", "Human Root event claim is malformed")
        if not isinstance(event.get("authorization_receipt_digest"), str) or not SHA256_RE.fullmatch(event["authorization_receipt_digest"]):
            raise V3Error("INVALID_EVIDENCE", "Human Root authorization receipt digest is invalid")
        if assurance["root"].get("assurance_level") not in {"signed_assertion", "trusted_time", "capture_attested"}:
            raise V3Error("INVALID_EVIDENCE", "root assurance level is invalid")
    else:
        if signer.get("role") != "final-attestor" or evidence.get("publish_ready") is not True:
            raise V3Error("INVALID_EVIDENCE", "Final evidence requires final-attestor and publish_ready=true")
        ...
        if not isinstance(event, dict) or set(event) != {"action", "source_kind", "transformations", "tool", "version"} or event.get("action") != "trusted-finalization":
            raise V3Error("INVALID_EVIDENCE", "Final event claim is malformed")
        if assurance["structural"].get("state") != "clean":
            raise V3Error("INVALID_EVIDENCE", "Final structural assurance must be clean")
        ...
        if mode == "standard" and no_unmapped is not False:
            raise V3Error("INVALID_EVIDENCE", "STANDARD cannot claim no unmapped generated content")
        if mode == "strict_origin" and no_unmapped is not True:
            raise V3Error("INVALID_EVIDENCE", "STRICT ORIGIN must sign its mapping guarantee")
        if mode not in {"standard", "strict_origin"}:
            raise V3Error("INVALID_EVIDENCE", "derivation mode is invalid")
        if mode == "strict_origin" and not isinstance(evidence.get("source_mapping"), dict):
            raise V3Error("INVALID_EVIDENCE", "STRICT ORIGIN source_mapping is required")
        if mode == "standard" and evidence.get("source_mapping") is not None:
            raise V3Error("INVALID_EVIDENCE", "STANDARD must not contain a Strict source mapping")


def load_v3_evidence(snapshot: Snapshot, core: Any, limits: dict[str, int | float]) -> dict[str, object]:
...
def verify_policy_claim(evidence: dict[str, object], policy: dict[str, object], policy_digest: str) -> None:
    ...
    if policy["mode"] == "production" and isinstance(signed_policy, dict) and signed_policy.get("mode") != "production":
        raise V3Error("DEVELOPMENT_EVIDENCE_REJECTED", "development evidence cannot pass Production prepublish")
    expected = policy_claim(policy, policy_digest)
    if signed_policy != expected:
        raise V3Error("POLICY_DIGEST_MISMATCH", "evidence was not created under this exact Trust Policy")


def verify_evidence_snapshot(snapshot: Snapshot, *, policy: dict[str, object], policy_digest: str, verifier: dict[str, object], timestamp_provider_id: str | None, store: SnapshotStore, limits: dict[str, int | float], core: Any) -> dict[str, object]:
    evidence = load_v3_evidence(snapshot, core, limits)
    verify_policy_claim(evidence, policy, policy_digest)
    expected_verifier = evidence["identities"]["verifier"]
    if expected_verifier.get("id") != verifier["id"]:
        raise V3Error("VERIFIER_IDENTITY_MISMATCH", "Evidence expects a different approved verifier")
    current_identity = verifier.get("verifier_identity", verifier.get("provider", verifier["id"]))
    if expected_verifier.get("identity") != current_identity:
        raise V3Error("VERIFIER_IDENTITY_MISMATCH", "Evidence expects a different verifier identity")
    with tempfile.TemporaryDirectory(prefix="verify-", dir=store.root) as work:
        verify_statement(evidence, verifier, policy=policy, limits=limits, workdir=Path(work))
...
def secure_source_path(base: Path, raw: object) -> Path:
    if not isinstance(raw, str) or not raw:
        raise V3Error("INVALID_SOURCE_MAP", "source path must be a non-empty string")
    candidate = Path(raw)
...
def compose_source_map(map_snapshot: Snapshot, *, map_original_path: Path, policy: dict[str, object], policy_digest: str, verifier: dict[str, object], timestamp_provider_id: str | None, root_evidence: dict[str, object], store: SnapshotStore, limits: dict[str, int | float], core: Any) -> dict[str, object]:
    ...
    if raw.get("schema_version") not in SOURCE_MAP_VERSIONS or raw.get("kind") not in {"text", "media"}:
        raise V3Error("INVALID_SOURCE_MAP", "unsupported source map schema or kind")
    source_values = raw.get("sources")
    if not isinstance(source_values, list) or not source_values:
        raise V3Error("SOURCE_MAP_INCOMPLETE", "source map requires signed Human sources")
    if len(source_values) > limits["source_count"]:
        raise V3Error("SOURCE_COUNT_EXCEEDED", "source count exceeds Policy limit")
    sources: dict[str, dict[str, object]] = {}
    summaries = []
    base = map_original_path.parent
    for item in source_values:
        if not isinstance(item, dict) or set(item) != {"source_id", "asset", "evidence"}:
            raise V3Error("INVALID_SOURCE_MAP", "source entries have unknown or missing fields")
        source_id = item.get("source_id")
        if not isinstance(source_id, str) or not SOURCE_ID_RE.fullmatch(source_id) or source_id in sources:
            raise V3Error("INVALID_SOURCE_MAP", "source_id must be unique and portable")
        ...
        if source_evidence["evidence_type"] != "human-root":
            raise V3Error("STRICT_SOURCE_NOT_HUMAN", "Strict sources must use Human Root evidence")
        ...
        if source_snapshot.sha256 != expected["sha256"] or source_snapshot.size != expected["size"]:
            raise V3Error("SOURCE_ASSET_MISMATCH", "source snapshot does not match signed Human Root", source_id=source_id)
    ...
    if not any(item["summary"]["evidence_digest"] == root_digest and item["summary"]["asset_id"] == root_evidence["asset"]["id"] for item in sources.values()):
        raise V3Error("SOURCE_MAP_ROOT_MISSING", "source map must include the supplied Human Root")
    ...
    if actor not in {"ai", "human", "tool", "mixed"}:
        raise V3Error("INVALID_SOURCE_MAP", "instruction_actor is invalid")
    assembled: bytes | None = None
    operations_out: list[dict[str, object]] = []
    if raw["kind"] == "text":
        operations = raw.get("operations")
        if not isinstance(operations, list) or not operations:
            raise V3Error("SOURCE_MAP_INCOMPLETE", "text source map requires operations")
        if len(operations) > limits["operation_count"]:
            raise V3Error("OPERATION_COUNT_EXCEEDED", "source map operation count exceeds Policy limit")
        ...
        if not isinstance(allowed_boundaries, list) or any(item not in {"grapheme", "token", "word", "line", "paragraph", "code_point"} for item in allowed_boundaries):
            raise V3Error("INVALID_POLICY", "slice boundary policy is invalid")
        if "code_point" in allowed_boundaries and not policy.get("slice_boundary_policy", {}).get("advanced_code_point", False):
            raise V3Error("INVALID_POLICY", "code_point slicing requires advanced_code_point=true")
        normalized: dict[str, str] = {}
        boundaries: dict[str, dict[str, set[int]]] = {}
        for source_id, source in sources.items():
            if source["evidence"]["asset"]["media_type"] not in TEXT_MEDIA_TYPES:
                raise V3Error("STRICT_TEXT_SOURCE_UNSUPPORTED", "Strict text sources must be TXT or Markdown")
            try:
                text = source["snapshot"].read_bytes().decode("utf-8", errors="strict")
            except UnicodeDecodeError as error:
                raise V3Error("INVALID_UTF8", "Strict text source is not UTF-8") from error
            try:
        ...
        for operation in operations:
            if not isinstance(operation, dict):
                raise V3Error("INVALID_SOURCE_MAP", "source map operation must be an object")
            if operation.get("op") == "separator":
                if set(operation) != {"op", "value"} or operation.get("value") not in ALLOWED_SEPARATORS:
                    raise V3Error("STRICT_LITERAL_FORBIDDEN", "only fixed whitespace separators are allowed")
            ...
            if operation.get("op") != "slice" or set(operation) - {"op", "source_id", "start", "end", "boundary"}:
                raise V3Error("INVALID_SOURCE_MAP", "unsupported text operation")
            ...
            if source_id not in normalized or not isinstance(start, int) or not isinstance(end, int) or boundary not in allowed_boundaries:
                raise V3Error("INVALID_SOURCE_MAP", "slice source/bounds/boundary are invalid")
            source_text = normalized[source_id]
            if start < 0 or end <= start or end > len(source_text):
                raise V3Error("SOURCE_MAP_RANGE_INVALID", "slice bounds are outside normalized source")
            if boundary != "code_point" and (start not in boundaries[source_id][boundary] or end not in boundaries[source_id][boundary]):
                raise V3Error("SLICE_BOUNDARY_VIOLATION", "slice splits the declared boundary", boundary=boundary)
        ...
        if one_grapheme_slices > 1 and not policy.get("slice_boundary_policy", {}).get("allow_letter_synthesis", False):
            raise V3Error("LETTER_SYNTHESIS_FORBIDDEN", "letter-by-letter semantic synthesis is forbidden by default")
        ...
        if primary not in sources:
            raise V3Error("SOURCE_MAP_INCOMPLETE", "media map primary_source_id is invalid")
...
def validate_json_shape(value: object, schema: dict[str, object], *, path: str = "$") -> None:
    ...
    if expected not in types or not isinstance(value, types[expected]) or (expected in {"number", "integer"} and isinstance(value, bool)):
        raise V3Error("JSON_SCHEMA_MISMATCH", "JSON value does not match pinned shape", path=path, expected=expected)
    if expected == "object":
        ...
        if not isinstance(properties, dict) or not isinstance(required, list) or not isinstance(additional, bool):
            raise V3Error("INVALID_JSON_SCHEMA", "origen-json-shape/1 object schema is malformed")
        if any(key not in value for key in required):
            raise V3Error("JSON_SCHEMA_MISMATCH", "JSON required property is missing", path=path)
        if not additional and set(value) - set(properties):
            raise V3Error("JSON_SCHEMA_MISMATCH", "JSON contains an unapproved property", path=path, properties=sorted(set(value) - set(properties)))
        ...
        if not isinstance(items, dict):
            raise V3Error("INVALID_JSON_SCHEMA", "array schema requires items")
...
def validate_json_with_policy(snapshot: Snapshot, schema_id: str | None, *, policy: dict[str, object], store: SnapshotStore, limits: dict[str, int | float], core: Any) -> dict[str, object]:
    ...
    if not schema_id or not isinstance(schemas, dict) or schema_id not in schemas:
        raise V3Error("JSON_SCHEMA_REQUIRED", "built-in JSON finalization requires an approved schema ID")
    descriptor = schemas[schema_id]
    if not isinstance(descriptor, dict) or set(descriptor) != {"path", "sha256"}:
        raise V3Error("INVALID_POLICY", "approved_json_schemas entry is malformed")
    ...
    if schema_snapshot.sha256 != require_sha256(descriptor["sha256"], "approved JSON schema hash"):
        raise V3Error("RESOURCE_HASH_MISMATCH", "JSON shape schema hash does not match Policy")
    ...
    if schema.get("schema_version") != "origen-json-shape/1":
        raise V3Error("INVALID_JSON_SCHEMA", "only origen-json-shape/1 is built in")
    try:
        ...
        raise V3Error("INVALID_JSON", "JSON input is invalid") from error
    shape = schema.get("shape")
    if not isinstance(shape, dict):
        raise V3Error("INVALID_JSON_SCHEMA", "origen-json-shape/1 requires an object shape")
...
def run_external_builder(source: Snapshot, output_path: Path, source_context: dict[str, object] | None, builder: dict[str, object], *, policy: dict[str, object], limits: dict[str, int | float], workdir: Path, media_type: str, mode: str) -> dict[str, object]:
    ...
    if response.get("status") != "built" or response.get("builder_id") != builder["id"]:
        raise V3Error("INVALID_BUILDER_RESPONSE", "approved builder did not confirm the expected build")
    return response
...
def external_inspect(snapshot: Snapshot, source_context: dict[str, object] | None, inspector: dict[str, object], *, policy: dict[str, object], limits: dict[str, int | float], workdir: Path, media_type: str, original_snapshot: Snapshot | None = None, c2pa_action: str | None = None) -> dict[str, object]:
    ...
    if response.get("status") != "inspected" or response.get("inspector_id") != inspector["id"]:
        raise V3Error("INVALID_INSPECTOR_RESPONSE", "approved inspector did not confirm the expected inspection")
    coverage = response.get("coverage")
    if not isinstance(coverage, dict) or set(coverage) != FINAL_COVERAGE:
        raise V3Error("INSPECTOR_COVERAGE_INCOMPLETE", "Final Inspector did not report every required property")
    ...
    if unknown or failed:
        raise V3Error("FINAL_INSPECTION_FAILED", "Final Inspector reported unknown or failed coverage", unknown=unknown, failed=failed)
    ...
    if not isinstance(signals, dict) or signals.get("state") not in CONTENT_SIGNAL_STATES or not isinstance(signals.get("checks"), list):
        raise V3Error("INVALID_INSPECTOR_RESPONSE", "content_signals result is malformed")
    if signals["state"] == "detected":
        raise V3Error("CONTENT_SIGNAL_DETECTED", "content-origin signal was detected and Policy rejects publication")
    if source_context is not None and not all(response.get(key) is True for key in ("operation_validated", "source_bindings_validated", "output_validated")):
        raise V3Error("STRICT_INSPECTION_INCOMPLETE", "independent inspector did not revalidate operation, sources, and output")
...
def builtin_final_inspection(snapshot: Snapshot, *, inspection: dict[str, object], c2pa_original: dict[str, object] | None = None) -> dict[str, object]:
    if inspection["findings"] or inspection["c2pa_markers"]:
        raise V3Error("FINAL_INSPECTION_FAILED", "built-in Final Inspector found prohibited structure", findings=inspection["findings"], c2pa=inspection["c2pa_markers"])
    if inspection["structural_provenance"] != "clean" or inspection["provenance_status"] != "clean":
        raise V3Error("FINAL_INSPECTION_UNKNOWN", "built-in Final Inspector cannot prove structural coverage")
...
def rename_directory_noreplace(source: Path, destination: Path) -> None:
    ...
    if sys.platform == "darwin" and hasattr(libc, "renamex_np"):
        ...
        raise V3Error("ATOMIC_NOREPLACE_UNAVAILABLE", "platform lacks an atomic no-replace directory rename")
    if result != 0:
        error_number = ctypes.get_errno()
        if error_number in {17, 39, 66}:
            raise V3Error("OUTPUT_EXISTS", "publish bundle already exists", path=str(destination))
...
def write_bundle_atomic(bundle: Path, asset: Snapshot, evidence: dict[str, object], receipt: dict[str, object]) -> None:
    ...
    if bundle.exists() or bundle.is_symlink():
        raise V3Error("OUTPUT_EXISTS", "publish bundle already exists", path=str(bundle))
...
def write_file_noreplace(path: Path, data: bytes) -> None:
    ...
    try:
        ...
        raise V3Error("OUTPUT_EXISTS", "refusing to overwrite an existing output", path=str(path)) from error
    try:
...
def policy_context(args: argparse.Namespace, store: SnapshotStore, core: Any) -> tuple[dict[str, object], str, dict[str, int | float]]:
    policy_snapshot = store.capture(Path(args.policy), label="Trust Policy", maximum=4 * 1024 * 1024)
    policy, policy_digest, limits = load_policy(policy_snapshot, core)
    return policy, policy_digest, limits
...
def command_root(args: argparse.Namespace, core: Any) -> dict[str, object]:
    with SnapshotStore() as store:
        ...
        signer = resolve_tool(policy, "approved_signers", args.signer_id, role="root-attestor")
        verifier = resolve_tool(policy, "approved_verifiers", args.verifier_id)
        creator_map = policy["creator_key_map"].get(args.creator_id)
        if not isinstance(creator_map, dict) or creator_map.get("key_id") != signer["key_id"] or creator_map.get("signer_identity") != signer["signer_identity"]:
            raise V3Error("CREATOR_KEY_MAPPING_MISMATCH", "creator_id is not mapped to the approved root key identity")
        local_time = normalize_timestamp(args.timestamp, label="local claimed time") if args.timestamp else timestamp_now()
        root_subject = {
            "creator_id": args.creator_id, "origin_id": args.origin_id,
            "asset_sha256": asset.sha256, "media_type": inspection["asset"]["media_type"],
            "policy_digest": policy_digest, "signer_key_id": signer["key_id"],
            "signer_identity": signer["signer_identity"], "local_claimed_time": local_time,
        }
        timestamp_statement: dict[str, object] | None = None
        timestamp_proof: dict[str, object] = {}
        timestamp_tool: dict[str, object] | None = None
        if args.timestamp_provider_id:
            provider = resolve_tool(policy, "approved_timestamp_providers", args.timestamp_provider_id)
            with tempfile.TemporaryDirectory(prefix="timestamp-", dir=store.root) as work:
                timestamp_statement, timestamp_proof = obtain_trusted_timestamp(root_subject, provider, policy=policy, limits=limits, workdir=Path(work))
            timestamp_tool = tool_claim(provider, identity_field="provider")
            assurance_level = "trusted_time"
        else:
            assurance_level = "signed_assertion"
        if policy["mode"] == "production" and assurance_level != "trusted_time":
            raise V3Error("TRUSTED_TIMESTAMP_REQUIRED", "Phase 1 Production Human Root requires an approved trusted timestamp provider")
        with tempfile.TemporaryDirectory(prefix="root-authorize-", dir=store.root) as work:
            authorization_digest = authorize_root(root_subject, signer, policy=policy, limits=limits, workdir=Path(work))
        ...
        statement: dict[str, object] = {
            ...
            "identities": {"signer": signer_identity(signer), "verifier": {"id": verifier["id"], "identity": verifier.get("verifier_identity", verifier.get("provider", verifier["id"]))}},
            "toolchain": {"signer": tool_claim(signer, identity_field="signer_identity"), "verifier": tool_claim(verifier, identity_field="verifier_identity"), "timestamp_provider": timestamp_tool},
            "timestamp": timestamp_statement or {"provider_id": None, "provider_identity": None, "protocol": None, "subject_sha256": digest_bytes(canonical_bytes(root_subject)), "trusted_time": None, "receipt_digest": None},
        ...
        evidence = {**statement, "proof": {**signature_proof, **timestamp_proof}}
...
def load_root_arg(args: argparse.Namespace, *, policy: dict[str, object], policy_digest: str, verifier: dict[str, object], store: SnapshotStore, limits: dict[str, int | float], core: Any) -> tuple[dict[str, object] | None, Snapshot | None]:
    if not args.root_evidence:
        if policy.get("root_required", True):
            raise V3Error("ROOT_REQUIRED", "Trust Policy requires a verified Human Root")
        return None, None
    ...
    if evidence["evidence_type"] != "human-root":
        raise V3Error("INVALID_ROOT", "root evidence must be human-root")
...
def command_strict_compose(args: argparse.Namespace, core: Any) -> dict[str, object]:
    with SnapshotStore() as store:
        policy, policy_digest, limits = policy_context(args, store, core)
        verifier = resolve_tool(policy, "approved_verifiers", args.verifier_id)
        root, _ = load_root_arg(args, policy=policy, policy_digest=policy_digest, verifier=verifier, store=store, limits=limits, core=core)
        if root is None:
            raise V3Error("STRICT_ROOT_REQUIRED", "strict-compose requires a verified Human Root")
        map_path = Path(args.source_map)
        map_snapshot = store.capture(map_path, label="source map", maximum=int(limits["source_map_bytes"]))
        context = compose_source_map(map_snapshot, map_original_path=map_path, policy=policy, policy_digest=policy_digest, verifier=verifier, timestamp_provider_id=args.timestamp_provider_id, root_evidence=root, store=store, limits=limits, core=core)
        if context["kind"] != "text" or context["assembled"] is None:
            raise V3Error("STRICT_COMPOSE_TEXT_ONLY", "strict-compose currently supports TXT and Markdown source maps")
...
def command_finalize(args: argparse.Namespace, core: Any) -> dict[str, object]:
    with SnapshotStore() as store:
        policy, policy_digest, limits = policy_context(args, store, core)
        signer = resolve_tool(policy, "approved_signers", args.signer_id, role="final-attestor")
        verifier = resolve_tool(policy, "approved_verifiers", args.verifier_id)
        root, _ = load_root_arg(args, policy=policy, policy_digest=policy_digest, verifier=verifier, store=store, limits=limits, core=core)
        ...
        if args.parent_evidence:
            ...
            parent = verify_evidence_snapshot(parent_snapshot, policy=policy, policy_digest=policy_digest, verifier=verifier, timestamp_provider_id=args.timestamp_provider_id, store=store, limits=limits, core=core)
        ...
        if media_type in UNSUPPORTED_FINAL_MEDIA_TYPES:
            raise V3Error("UNSUPPORTED_FORMAT", "this format has no Phase 1 finalize contract", media_type=media_type)
        mode = args.guarantee_level
        source_context: dict[str, object] | None = None
        if mode == "strict_origin":
            if root is None or not args.source_map:
                raise V3Error("SOURCE_MAP_REQUIRED", "STRICT ORIGIN requires Human Root and source map")
            map_path = Path(args.source_map)
            map_snapshot = store.capture(map_path, label="source map", maximum=int(limits["source_map_bytes"]))
            source_context = compose_source_map(map_snapshot, map_original_path=map_path, policy=policy, policy_digest=policy_digest, verifier=verifier, timestamp_provider_id=args.timestamp_provider_id, root_evidence=root, store=store, limits=limits, core=core)
            if source_context["kind"] == "text":
                if media_type not in TEXT_MEDIA_TYPES:
                    raise V3Error("STRICT_TEXT_TYPE_UNSUPPORTED", "Strict text supports TXT and Markdown")
                try:
                    proposed = core.canonical_text_output(source.read_bytes().decode("utf-8", errors="strict"))
                except (UnicodeDecodeError, core.OrigenError) as error:
                    raise V3Error("INVALID_UTF8", "Strict final text input is invalid") from error
                if proposed != source_context["assembled"]:
                    raise V3Error("STRICT_CONTENT_MISMATCH", "final text was not rebuilt solely from signed Human source mapping")
            else:
                primary = source_context["sources"][source_context["primary_source_id"]]
                if source.sha256 != primary["snapshot"].sha256:
                    raise V3Error("STRICT_CONTENT_MISMATCH", "Strict media input is not the signed primary source snapshot")
        elif args.source_map:
            raise V3Error("SOURCE_MAP_NOT_ALLOWED", "STANDARD does not accept a Strict source map")
        ...
        if external_required:
            ...
            if builder["id"] == inspector["id"] or (builder_files and builder_files == inspector_files):
                raise V3Error("INSPECTOR_NOT_INDEPENDENT", "builder and Final Inspector must be independently approved identities")
        ...
        if input_inspection["c2pa_markers"]:
            ...
            if action not in {"preserve", "reissue", "detach"} or inspector is None:
                raise V3Error("C2PA_POLICY_REQUIRED", "embedded/external C2PA requires explicit action and approved external Inspector")
        ...
        with tempfile.TemporaryDirectory(prefix="finalize-", dir=store.root) as work_raw:
            ...
            if source_context and source_context["kind"] == "text":
                ...
                try:
                    ...
                    raise V3Error(error.code, error.message, **error.details) from error
                if adapter is None:
                    raise V3Error("BUILDER_REQUIRED", "format requires an approved external builder")
            ...
            if final_media_type != media_type:
                raise V3Error("FINAL_MEDIA_TYPE_MISMATCH", "builder changed media type")
            if inspector is None:
                inspection_result = builtin_final_inspection(output_snapshot, inspection=final_inspection)
                inspector_claim = {"id": "origen/builtin-final-inspector", "identity": "origen/builtin-0.3", "coverage": sorted(FINAL_COVERAGE)}
            else:
            ...
            if c2pa_record is not None:
                ...
                if not isinstance(external_c2pa, dict) or external_c2pa.get("original_status") != "valid" or not isinstance(external_c2pa.get("original_manifest_digest"), str):
                    raise V3Error("C2PA_VALIDATION_REQUIRED", "external Inspector did not validate original C2PA provenance")
                if external_c2pa.get("action") != c2pa_record["action"]:
                    raise V3Error("C2PA_ACTION_MISMATCH", "Inspector C2PA action differs from Policy")
            ...
            if representation not in handoff["publication_representations"]:
                raise V3Error("PUBLICATION_REPRESENTATION_NOT_ALLOWED", "publication representation is not allowed by Policy")
            local_time = normalize_timestamp(args.timestamp, label="local claimed time") if args.timestamp else timestamp_now()
            derivation = {
                "mode": mode,
            ...
            statement: dict[str, object] = {
                ...
                "identities": {
                    "signer": signer_identity(signer),
                    "verifier": {"id": verifier["id"], "identity": verifier.get("verifier_identity", verifier.get("provider", verifier["id"]))},
                    "builder": {"id": builder_claim["id"], "identity": builder_claim["identity"]},
                    "inspector": {"id": inspector_claim["id"], "identity": inspector_claim["identity"]},
                },
                "toolchain": {
                    "signer": tool_claim(signer, identity_field="signer_identity"),
                    "verifier": tool_claim(verifier, identity_field="verifier_identity"),
...
def load_bundle(bundle: Path, store: SnapshotStore, limits: dict[str, int | float], core: Any) -> tuple[Snapshot, Snapshot, dict[str, object], dict[str, object]]:
    ...
    evidence = load_v3_evidence(evidence_snapshot, core, limits)
    ...
    if set(receipt) != expected_fields:
        raise V3Error("INVALID_RECEIPT", "bundle receipt has unknown or missing fields")
    return asset, evidence_snapshot, evidence, receipt


def verify_lineage_arg(path: str | None, *, label: str, expected_digest: object, expected_asset_id: object, policy: dict[str, object], policy_digest: str, verifier: dict[str, object], timestamp_provider_id: str | None, store: SnapshotStore, limits: dict[str, int | float], core: Any) -> dict[str, object] | None:
    if expected_digest is None and expected_asset_id is None:
        if path:
            raise V3Error("UNEXPECTED_LINEAGE", f"{label} evidence was supplied without a signed link")
        return None
    if not path:
        raise V3Error("LINEAGE_INCOMPLETE", f"signed {label} evidence is required")
    ...
    if evidence_digest(evidence) != expected_digest or evidence["asset"]["id"] != expected_asset_id:
        raise V3Error("LINEAGE_MISMATCH", f"{label} evidence does not match signed lineage")
    return evidence
...
def command_verify(args: argparse.Namespace, core: Any, *, prepublish: bool) -> dict[str, object]:
    with SnapshotStore() as store:
        ...
        if asset.sha256 != expected_asset["sha256"] or asset.size != expected_asset["size"]:
            raise V3Error("ASSET_MISMATCH", "bundle asset does not match signed Final Evidence")
        if receipt["asset_sha256"] != asset.sha256 or receipt["evidence_digest"] != evidence_digest(evidence) or receipt["policy_digest"] != policy_digest:
            raise V3Error("RECEIPT_MISMATCH", "bundle receipt does not bind exact asset, evidence, and Policy")
        if receipt["guarantee_mode"] != evidence["assurance"]["derivation"]["mode"] or receipt["media_type"] != expected_asset["media_type"]:
            raise V3Error("RECEIPT_MISMATCH", "bundle receipt guarantee/media type differs from Evidence")
        if receipt["publication_representation"] != evidence["publication"]["representation"] or receipt["allowed_transport_metadata"] != evidence["publication"]["allowed_transport_metadata"]:
            raise V3Error("RECEIPT_MISMATCH", "bundle receipt publication contract differs from Evidence")
        lineage = evidence["lineage"]
        root = verify_lineage_arg(args.root_evidence, label="root", expected_digest=lineage.get("root_evidence_digest"), expected_asset_id=lineage.get("root_asset_id"), policy=policy, policy_digest=policy_digest, verifier=verifier, timestamp_provider_id=args.timestamp_provider_id, store=store, limits=limits, core=core)
        parent = verify_lineage_arg(args.parent_evidence, label="parent", expected_digest=lineage.get("parent_evidence_digest"), expected_asset_id=lineage.get("parent_asset_id"), policy=policy, policy_digest=policy_digest, verifier=verifier, timestamp_provider_id=args.timestamp_provider_id, store=store, limits=limits, core=core)
        if policy.get("root_required", True) and root is None:
            raise V3Error("ROOT_REQUIRED", "Production Policy requires linked Human Root")
        if parent and parent["lineage"].get("root_evidence_digest") and root and parent["lineage"]["root_evidence_digest"] != evidence_digest(root):
            raise V3Error("LINEAGE_MISMATCH", "parent and supplied Human Root disagree")
        if evidence["assurance"]["derivation"]["mode"] == "strict_origin":
            if not args.source_map or root is None:
                raise V3Error("SOURCE_MAP_REQUIRED", "STRICT ORIGIN prepublish requires source map and Human Root")
            map_path = Path(args.source_map)
            map_snapshot = store.capture(map_path, label="source map", maximum=int(limits["source_map_bytes"]))
            context = compose_source_map(map_snapshot, map_original_path=map_path, policy=policy, policy_digest=policy_digest, verifier=verifier, timestamp_provider_id=args.timestamp_provider_id, root_evidence=root, store=store, limits=limits, core=core)
            if canonical_bytes(context["summary"]) != canonical_bytes(evidence["source_mapping"]):
                raise V3Error("SOURCE_MAP_MISMATCH", "source map summary differs from signed Final Evidence")
            if context["kind"] == "text":
                ...
                if rebuilt_digest != asset.sha256 or evidence["source_mapping"].get("rebuilt_output_sha256") != asset.sha256:
                    raise V3Error("REBUILT_DIGEST_MISMATCH", "source snapshots rebuild to bytes different from Final asset")
        if prepublish and evidence.get("publish_ready") is not True:
            raise V3Error("NOT_PUBLISH_READY", "signed Final Evidence is not publish-ready")
...
def parser() -> argparse.ArgumentParser:
    root = argparse.ArgumentParser(prog="origen", description="Policy-enforced Content Origin / Provenance Trust Gate")
    ...
    def add_policy(command: argparse.ArgumentParser) -> None:
        ...
        command.add_argument("--verifier-id", required=True)
        command.add_argument("--timestamp-provider-id")

    inspect = commands.add_parser("inspect")
    inspect.add_argument("asset")
    inspect.add_argument("--policy", required=True)
    ...
    root_command.add_argument("--signer-id", required=True)
    ...
    finalize.add_argument("--signer-id", required=True)
    ...
    finalize.add_argument("--source-kind", required=True, choices=("ai-output", "external-tool", "human-edit", "captured-original"))
    ...
    finalize.add_argument("--publication-representation", required=True)
